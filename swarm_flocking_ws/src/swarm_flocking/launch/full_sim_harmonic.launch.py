...
def _spawn_all_harmonic(context, *args, **kwargs):
    ...
    if not waypoints:
      if world_name == 'open_field':
        ...
        # WP2 sits in the safe corridor between maze walls to reduce side bypassing.
        waypoints = [8.5, 7.5, 24.8, 9.9, 28.0, 7.5]
      else:
          ...
          RosNode(
            ...
            parameters=[
              params_file,
              {
                ...
                'waypoint_sync_fraction': 0.05 if not enable_obstacle_avoidance else 0.90,
                ...
                'w_migration': 1.35 if not enable_obstacle_avoidance else 0.58,
                'sync_position_gain': 2.6 if not enable_obstacle_avoidance else 1.65,
                'sync_velocity_gain': 0.90 if not enable_obstacle_avoidance else 0.72,
                'sync_max_w': 3.4 if not enable_obstacle_avoidance else 2.8,
                'sync_leader_id': 2 if not enable_obstacle_avoidance else 4,
                'sync_obstacle_relax': 0.20 if not enable_obstacle_avoidance else 0.45,
                'progress_timeout_s': 20.0 if not enable_obstacle_avoidance else 5.0,
                'desync_duration_s': 0.5 if not enable_obstacle_avoidance else 1.2,
                'desync_sync_scale': 0.9 if not enable_obstacle_avoidance else 0.60,
                'desync_migration_boost': 1.05 if not enable_obstacle_avoidance else 1.4,
                'stall_escalation_gain': 0.0 if not enable_obstacle_avoidance else 0.15,
                'regroup_gain': 1.4 if not enable_obstacle_avoidance else 1.1,
                'max_regroup_w': 3.2 if not enable_obstacle_avoidance else 2.0,
                'max_linear_vel': 0.22 if not enable_obstacle_avoidance else 0.18,
                'front_slowdown_distance': 1.0 if not enable_obstacle_avoidance else 1.15,
                'front_stop_distance': 0.26 if not enable_obstacle_avoidance else 0.36,
    ...
    monitor = RosNode(
        ...
        parameters=[
            params_file,
            {
                ...
                'use_sim_time': use_sim_time == 'true',
              'waypoint_bottleneck_guard_enable': enable_obstacle_avoidance,
              'waypoint_reach_fraction': 0.40 if not enable_obstacle_avoidance else 0.84,
              'waypoint_reach_radius': 2.2 if not enable_obstacle_avoidance else 1.25,
              'goal_tolerance': 1.8 if not enable_obstacle_avoidance else 1.0,
