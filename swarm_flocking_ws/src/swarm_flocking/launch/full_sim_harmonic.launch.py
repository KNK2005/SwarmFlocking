...
def _spawn_all_harmonic(context, *args, **kwargs):
    ...
    if not waypoints:
      if world_name == 'open_field':
        ...
        # Obstacle-course guidance: bottleneck center gap -> maze upper corridor -> final goal.
        # WP1 is centered in the first bottleneck opening at x=8.5, y=7.5.
        # WP2 sits deep inside the upper maze corridor so robots commit to the same passage.
        waypoints = [8.5, 7.5, 25.6, 9.85, 28.0, 7.5]
      else:
          ...
          RosNode(
            ...
            parameters=[
              params_file,
              {
                ...
                'waypoint_sync_fraction': 0.05 if not enable_obstacle_avoidance else 1.00,
                'waypoint_sync_radius_scale': 1.60 if not enable_obstacle_avoidance else 1.20,
                'waypoint_bottleneck_guard_margin': 0.25 if not enable_obstacle_avoidance else 0.40,
                'w_separation': 0.30 if not enable_obstacle_avoidance else 1.15,
                'w_alignment': 2.2 if not enable_obstacle_avoidance else 1.4,
                'w_cohesion': 3.0 if not enable_obstacle_avoidance else 1.6,
                'w_migration': 1.35 if not enable_obstacle_avoidance else 0.62,
                'sync_position_gain': 2.6 if not enable_obstacle_avoidance else 2.10,
                'sync_velocity_gain': 0.90 if not enable_obstacle_avoidance else 0.85,
                'sync_max_w': 3.4 if not enable_obstacle_avoidance else 3.2,
                'sync_leader_id': 2 if not enable_obstacle_avoidance else 4,
                'sync_obstacle_relax': 0.20 if not enable_obstacle_avoidance else 0.25,
                'sync_spacing_x': spacing_x if not enable_obstacle_avoidance else 0.78,
                'sync_spacing_y': spacing_y if not enable_obstacle_avoidance else 0.42,
                'progress_timeout_s': 20.0 if not enable_obstacle_avoidance else 5.0,
                'desync_duration_s': 0.5 if not enable_obstacle_avoidance else 0.8,
                'desync_sync_scale': 0.9 if not enable_obstacle_avoidance else 0.75,
                'desync_migration_boost': 1.05 if not enable_obstacle_avoidance else 1.4,
                'stall_escalation_gain': 0.0 if not enable_obstacle_avoidance else 0.05,
                'regroup_gain': 1.4 if not enable_obstacle_avoidance else 1.3,
                'max_regroup_w': 3.2 if not enable_obstacle_avoidance else 2.0,
                'max_linear_vel': 0.22 if not enable_obstacle_avoidance else 0.16,
                'front_slowdown_distance': 1.0 if not enable_obstacle_avoidance else 1.25,
                'front_stop_distance': 0.26 if not enable_obstacle_avoidance else 0.40,
                'wall_follow_gain': 1.1 if not enable_obstacle_avoidance else 0.75,
                'wall_follow_max_w': 1.6 if not enable_obstacle_avoidance else 1.1,
                'goal_projection_min': 0.45 if not enable_obstacle_avoidance else 1.05,
                'goal_projection_gain': 1.2 if not enable_obstacle_avoidance else 1.9,
                'goal_projection_max_boost': 1.4 if not enable_obstacle_avoidance else 2.6,
                'goal_projection_obs_relax': 0.85 if not enable_obstacle_avoidance else 1.8,
                'context_bottleneck_min_scan': 0.55 if not enable_obstacle_avoidance else 0.40,
              },
            ],
            output='screen',
          ),
    ...
    monitor = RosNode(
        ...
        parameters=[
            params_file,
            {
              ...
              'waypoint_bottleneck_guard_enable': enable_obstacle_avoidance,
              'waypoint_bottleneck_guard_margin': 0.25 if not enable_obstacle_avoidance else 0.40,
              'waypoint_reach_fraction': 0.40 if not enable_obstacle_avoidance else 1.00,
              'waypoint_reach_radius': 2.2 if not enable_obstacle_avoidance else 1.00,
              'goal_tolerance': 1.8 if not enable_obstacle_avoidance else 0.85,
